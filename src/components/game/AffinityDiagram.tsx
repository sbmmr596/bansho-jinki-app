      ...
      <div className={cn("flex items-stretch gap-2", compact ? "gap-1.5" : "gap-3")}>
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-between gap-1">
          <div className="rounded-lg bg-raised/50 p-1.5 hairline">
            <p className="mb-1 text-center text-[13px] tracking-wide text-faint">無・天・地</p>
            <div className="flex items-center justify-center gap-1">
              <Chip type="void" />
              <span className="text-[13px] text-brass">⇔</span>
          ...
          <div className="rounded-md bg-ink/80 px-2 py-1 text-[13px] leading-snug text-muted hairline">
            <p>
          ...
          <div className="grid grid-cols-3 gap-0.5 text-[12px] leading-tight text-muted">
...
function Chip({ type, className }: { type: ElementType; className?: string }) {
  return (
    <div className={cn("z-10 flex flex-col items-center", className)}>
      ...
      <span className="mt-0.5 text-[13px] text-fg">{TYPE_LABEL[type]}</span>
    </div>
  );
}
