            ...
            <div className="flex gap-2.5">
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-0.5 text-xs">
                ...
                <div className="mt-0.5 flex flex-wrap items-center gap-1.5">
                  ...
                  {card.fodder ? (
                    <span className="rounded-sm bg-crimson/85 px-1.5 py-0.5 text-[14px] font-semibold text-fg">
                      素材専用
                    </span>
                  ) : null}
                </div>
              ...
              {!card.fodder ? (
                <div className="flex items-start gap-2 rounded-md bg-raised/50 p-1.5 hairline">
                  <div className="min-w-0 flex-1">
                    <p className="text-[14px] tracking-wide text-faint">リーダー陣形</p>
                    <p className="font-display text-sm leading-snug text-fg">{form?.name ?? "—"}</p>
                    <p className="mt-0.5 text-[15px] leading-snug text-muted">{form?.desc}</p>
                  </div>
  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between gap-2">
        <p className="text-[14px] tracking-wide text-faint">{label}</p>
        {empty ? (
          <span className="text-[14px] tabular text-crimson">Lv.-</span>
        ) : lv != null ? (
          <span className="text-[14px] tabular text-fg">Lv.{lv}</span>
        ) : null}
      </div>
      ...
      {power != null && !empty ? (
        <p className="mt-0.5 text-[14px] text-brass">
          威力:{scaled ?? power}
