...
export function TrainScreen() {
  ...
  return (
    <Shell title="合成・育成" extra={<GoldChip gold={gold} />} bg="/bg/palace.jpg" nav="train" wide>
      {/* Narrow container: one scroll column so スキル合成 stays reachable.
          @md+ (stage 1280 always): 3:3:4 (大カード / 技 / トレイ). */}
      <div className="grid h-full min-h-0 grid-cols-1 gap-2 overflow-y-auto overscroll-contain @md:grid-cols-[minmax(0,3fr)_minmax(0,3fr)_minmax(0,4fr)] @md:gap-3 @md:overflow-hidden">
            ...
            <>
              <div className="flex flex-col items-center gap-1.5">
                ...
                <div className="w-full space-y-0.5 text-center">
                  ...
                  <p className="text-[14px] text-muted">{card.title}</p>
                  <div className="flex flex-wrap items-center justify-center gap-1.5">
                    <TypeHex type={card.type} className="h-6 w-7 text-xs" />
                    <CostHex cost={card.cost} className="h-6 w-7 text-xs" />
                    <span className="text-[14px] text-brass">{card.rarity}</span>
                    {card.fodder ? (
                      <span className="rounded-sm bg-crimson/85 px-1 py-0.5 text-[13px] font-semibold text-fg">
                        素材専用
                      </span>
                    ) : null}
              ...
              <div className="rounded-md bg-raised/60 p-2 hairline">
                <div className="mb-1.5 grid grid-cols-3 items-center gap-x-2 gap-y-1 text-[14px]">
                ...
                <div className="grid grid-cols-3 gap-1 text-center text-[14px]">
              ...
              {isFodderBase ? (
                <p className="rounded-sm bg-crimson/20 px-1.5 py-1 text-[15px] text-crimson">
                  素材専用 — 金鍛錬・同名合成不可（異名の素材には使える）
                </p>
              ) : lvMax ? (
                <p className="text-[15px] text-brass">レベルは最大。</p>
              ) : (
                <PrimaryButton
              ...
              {form && !card.fodder ? (
                <div className="rounded-md bg-raised/40 p-2 text-[14px] hairline">
        ...
        <div className="flex min-w-0 flex-col gap-2 @md:min-h-0 @md:overflow-y-auto">
          {card && own ? (
            <>
              ...
              {mode === "skill1" ? (
                <div className="rounded-md bg-raised/50 p-2 text-[15px] hairline">
              ...
              {lastResult ? (
                <p
                  className={cn(
                    "rounded-md px-2 py-1.5 text-[15px]",
        ...
        <div className="flex min-h-[9rem] shrink-0 flex-col pb-[max(0.25rem,env(safe-area-inset-bottom,0px))] @md:min-h-0 @md:pb-0">
          {mode === "skill2" ? (
            <>
              <p className="mb-1 text-[14px] text-muted">素材カード</p>
              <div className="grid max-h-[40%] auto-rows-min grid-cols-4 justify-items-stretch gap-x-0.5 gap-y-1 overflow-y-auto content-start @sm:grid-cols-5 @md:max-h-none @md:min-h-0 @md:flex-1 @md:grid-cols-5">
                {otherMaterials.length === 0 ? (
                  <p className="col-span-full text-[15px] text-faint">他に所持カードがない。</p>
                ) : (
                  otherMaterials.map((c) => (
                    ...
                    >
                      <CardFace
                        ...
                        className="w-full"
                      />
                      <p className="mt-0.5 w-full truncate text-center text-[14px] font-medium leading-tight text-fg">
                        {materialSkillLabel(c.skill, c.rarity)}
                      </p>
                      <p className="text-center text-[13px] tabular text-muted">
                        ×{owned[c.id]?.count ?? 0}
                      </p>
                    </button>
                  ))
            ...
            <>
              <p className="mb-1 text-[14px] text-muted">ベースカード</p>
              <div className="grid max-h-[40%] auto-rows-min grid-cols-4 justify-items-stretch gap-x-0.5 gap-y-1 overflow-y-auto content-start @sm:grid-cols-5 @md:max-h-none @md:min-h-0 @md:flex-1 @md:grid-cols-5">
                {tray.map((c) => (
                  <div key={c.id} className="flex w-full min-w-0 flex-col items-stretch">
                    ...
                    <p
                      className={cn(
                        "mt-0.5 w-full truncate text-center text-[14px] font-medium tabular",
...
function ModeChip({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "h-11 w-full rounded-md px-2 text-[15px] font-medium",
